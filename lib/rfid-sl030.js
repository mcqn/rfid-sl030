...
RFID_SL030.prototype.authenticate = function(aSector, aKey) {
    var data = new Buffer(8);
    data[0] = aSector;
    data[1] = 0xAA;
    if (aKey) {
        console.log("Authenticating with a given key");
        aKey.copy(data, 2);
    } else {
        console.log("Authenticating with a default key");
        data[2] = 0xFF;
        data[3] = 0xFF;
        data[4] = 0xFF;
        data[5] = 0xFF;
...
RFID_SL030.prototype.writePage = function(aPage, aData) {
    var data = new Buffer(1+4);
    data[0] = aPage;
    aData.copy(data, 1);
    this.sendCommand(this.kCmdWrite4, data);
    sleep.usleep(50000);
    // We'll get a 7 byte response to this
    // 3 bytes of header, and 4 bytes of data
    var respHeader = struct({
        len: ref.types.byte,
        command: ref.types.byte,
        status: ref.types.byte
    });
    var resp = new Buffer(7);
    var ret = (this.libbcm2835.bcm2835_i2c_read(resp, resp.length));
    if (ret == 0) {
        // Got a response
        var hdr = respHeader(resp.slice(0,3));
        ...
        // Sanity check the response
        if ((hdr.len >= 2) && (hdr.command == this.kCmdWrite4)) {
            if (hdr.status == 0) {
                return true;
            } else if (hdr.status == 1) {
                console.log("No tag present");
...
RFID_SL030.prototype.readPage = function(aPage) {
    var data = new Buffer(1);
    data[0] = aPage;
    this.sendCommand(this.kCmdRead4, data);
    sleep.usleep(50000);
    // We'll get an 7 byte response to this
    // 3 bytes of header, and 4 bytes of data
    var respHeader = struct({
        len: ref.types.byte,
        command: ref.types.byte,
    ...
    if (ret == 0) {
        ...
        // Sanity check the response
        if ((hdr.len >= 2) && (hdr.command == this.kCmdRead4)) {
            if (hdr.status == 0) {
                return resp.slice(3,3+4);
            } else if (hdr.status == 1) {
                console.log("No tag present");
            } else if (hdr.status == 0x04) {
                console.log("Read failed");
